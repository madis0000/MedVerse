...
export function PrescriptionPreview({
  ...
  async function handleDownloadPdf() {
    if (!prescriptionId) {
      toast.error('Prescription must be saved before downloading PDF');
      return;
    }
    ...
    <Card className="max-w-2xl mx-auto">
      ...
      <CardContent>
        ...
        <div className="text-center mb-4">
          <h2 className="text-xl font-bold text-foreground">MedPulse Clinic</h2>
          <p className="text-xs text-muted-foreground">
            123 Medical Center Drive, Suite 100 | Phone: (555) 123-4567
          </p>
        </div>
            ...
            {items.map((item, index) => (
              <div key={item.id ?? index} className="pl-4 border-l-2 border-primary/30">
                ...
                <div className="text-sm text-muted-foreground mt-1 space-y-0.5">
                  <p>Dosage: {item.dosage} | Route: {item.route}</p>
        ...
        <div className="mt-8 text-right">
          <div className="inline-block text-center">
            ...
            <p className="text-xs text-muted-foreground">Prescribing Physician</p>
          </div>
        </div>
      </CardContent>
    </Card>
