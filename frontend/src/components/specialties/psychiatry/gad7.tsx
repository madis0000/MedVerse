...
export function GAD7({ consultationId, patientId, readOnly = false }: GAD7Props) {
  const { t } = useTranslation();
  const instrument = getInstrument('GAD7')!;
  ...
  const fetchHistory = useCallback(async () => {
    try {
      ...
      const existing = records.find((r) => r.instrumentType === 'GAD7');
      if (existing) {
          ...
          <CardHeader>
            <div className="flex items-center justify-between">
              <CardTitle className="text-base">
                {t('specialties.psychiatry.gad7History')}
              </CardTitle>
                    ...
                    content={({ active, payload }) => {
                      ...
                      return (
                        <div className="rounded-md border bg-popover p-2 text-sm shadow-md">
                          ...
                          <p>
                            {t('specialties.psychiatry.score')}: {data.score}/21
                          </p>
